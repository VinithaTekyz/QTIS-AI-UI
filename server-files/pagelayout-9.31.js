...
Vue.component('cnx-page', {
	...
	methods: {
		...
		downloadInvoice() {
				executeAppAPI("chataifiledownloadapi", { input: {fileName:"invoice_001_1759848103189.docx"} }, null, this.handleFileDownload);
		},
		...
		async sendMessage(quickQuestion = '') {
			...
			// Check for quick question commands
			if (!this.trainingMode && (
				/\b(create user| create system user | add user| create client| add client| record client| add note| add notes | create note| create notes | add case| add cases| create case)\b/i.test(quickQuestion))) {
