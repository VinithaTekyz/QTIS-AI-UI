...
Vue.component('cnx-page', {
						...
						<q-card-actions align="center" class="q-pa-sm bg-grey-2 custom-field"
							...
							<q-input outlined dense autogrow rounded class="col" v-model="newMessage"
									...
									<q-btn dense flat round icon="add" color="grey-7">
										<q-menu auto-close>
											<q-list style="min-width:150px">
												...
												<q-item clickable v-ripple @click="setPlatformSearchModek()">
													<q-item-section avatar><q-icon name="upload" /></q-item-section>
													<q-item-section>Platform Search</q-item-section>
												</q-item>
												<hr style="width:85%; background-color: #9ea2a5;" />
												<!-- User -->
								...
								<template v-slot:append>
									...
									<q-chip v-if="platformSearchMode" clickable outline color="red" text-color="red" size="xs"
										style="margin-top:6px; max-width: fit-content;">
										Platform Search
									</q-chip>
								</template>
	...
	methods: {
		...
		setPlatformSearchModek() {
			this.platformSearhMode = true;
		},
		setTrainingMode(value) {
			this.trainingMode = value;
			this.platformSearhMode = false;
		},
		setMode(mode) {
			this.clearChat();
			if (mode === 'assistant') {
		...
		async sendMessage(quickQuestion = '') {
			...
			if (
				!this.trainingMode &&
				/\b(create user|create a user|add a user|add an user|create system user|add user|create client|create a client|add client|add a client|record client|record a client|add note|add a note|add notes|create note|create a note|create notes|add case|add a case|add cases|create case|create a case|add an event|add event|add a event|create event|create an event|create a event|clock in|clock out|clock me in|clock me out)\b/i.test(quickQuestion) &&
				!/\b(how|where|when|what)\b/i.test(quickQuestion)
			) {
				const type = quickQuestion.includes("clock") ? 'clock' :
					quickQuestion.includes("user") ? 'users' :
						quickQuestion.includes("client") ? 'clients' :
							quickQuestion.includes("case") ? 'cases' :
								quickQuestion.includes("note") ? 'notepad' : '';
				if (quickQuestion.toLocaleLowerCase().includes('user')) {
					this.selectedType = "users"
				} else if (quickQuestion.toLocaleLowerCase().includes("client")) {
					this.selectedType = "clients"
				} else if (quickQuestion.toLocaleLowerCase().includes("note")) {
					this.selectedType = "notepad"
				} else if (quickQuestion.toLocaleLowerCase().includes("case")) {
					this.selectedType = "cases"
				} else if (quickQuestion.toLocaleLowerCase().includes("event")) {
					this.selectedType = "event"
				} else if (quickQuestion.toLocaleLowerCase().includes("clock") && quickQuestion.toLocaleLowerCase().includes("in")) {
					this.selectedType = "clockIn"
				} else if (quickQuestion.toLocaleLowerCase().includes("clock") && quickQuestion.toLocaleLowerCase().includes("out")) {
				...
				if (this.trainingMode) {
					...
					parameters = {
						messages: [
							{
								"role": "system",
								"content": this.attachedFiles.length == 0 ? `chat_agent:${this.caseId}:platformSearch` : `file_summarization`
							},
							...formattedMessages,
							{
								"role": "token",
			...
			if (response.status == 'OK') {
				...
				if (match && match[0].includes('.docx')) {
					reply = `<p>Download the generated file:</p>
							<a href="javascript:void(0);" onclick="window.__vueHandle('${match[0]}', 'DOWNLOAD_INVOICE')">
							Download ${match[0].includes("invoice") ? "Invoice" : "Chat Summary"}
							</a>`
				} else if (match && match[0].includes('.pdf')) {
					reply = `<p>Download the generated file: </p>
							<a href="http://3.80.147.19/public/${match[0]}" target="_blank">
							Download ${match[0].includes("invoice") ? "Invoice" : "Chat Summary"}
							</a>`
				}
				this.messages[this.botIndex].text = reply;
